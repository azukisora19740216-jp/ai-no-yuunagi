...
- 状態: 実装進行中。Phase 0および利用者依頼上のフェーズ1・2を実装済み。ADR-0004・ADR-0005のPD-01〜PD-07は実装済み・PostgreSQL統合検証済み。後続の外部確認、運用準備およびMVP未実装範囲は継続中。
...
- 検証日: 2026-07-25
- 対象SHA: `1a8e19edcb9aa3cf3c6b670a8484db6f8e01fd7f`
- 証跡: [CI #55 / attempt 2](https://github.com/azukisora19740216-jp/ai-no-yuunagi/actions/runs/30035051625)
- 状態: PD-01〜PD-07は実装済み・PostgreSQL統合検証済み

CIでは、frozen lockfile install、全migration、seed、単体40件、PostgreSQL 18.4統合27件、Desktop/Mobile E2E 12件、production build、container buildが成功し、skipは0件だった。lockfile再生成および供給網ポリシーの緩和は行っていない。

Mobile E2Eのログイン遷移と管理画面遷移は、それぞれ1回のretry後に成功した。既知のflaky testとして記録し、クローズドテスト開始前に安定化する。
...
**状態（2026-07-25更新）:** 実装済み。CI #55のPostgreSQL 18.4サービスでmigration、統合テストおよび主要E2Eを検証済み。作業端末上のCompose実起動有無とは分離して記録する。
...
**状態（2026-07-25更新）:** 取引付与、保留、反対仕訳、共通プールmovement、履歴・管理UI、1年後の月末失効、60/30/7日前通知予定、利用可能残高30上限、上限超過分の原子的pool移行、開発データ除外を実装済み・PostgreSQL統合検証済み。残高は直接保存せず、追記型台帳とmovementから集計する。通知送信workerと再送運用は未実装。
...
C-102〜C-104およびC-129Aの残高・配分詳細を決定するまでポイント配分を無効にする。失効・上限はADR-0005に従う非破壊migrationで実装済みだが、本番開始日時と初期point policyが承認されるまで正式付与経路はfail closedとする。
